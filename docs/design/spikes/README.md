...
|---|---|---|---|---|
| How does a hot-path publisher name its output? (#9) | **Static topology: a plain publisher naming the `StaticWiring` alias.** Dynamic topology: the CRTP mixin `sub0x::Publisher<Derived, Out>`, holding the wiring by value. A TU or library boundary: `Sink<T>` | Direct alias `=` against `handwritten`. CRTP mixin, hand-spelled template and call-site argument `=` against hand-written runtime binding; `Sink<T>` `=` against hand-written type erasure (context + function pointer), on every build | Call-site argument (`=`). Rejected: the CTAD factory (clang +4 instr, +24 B: clang cannot see through its by-value return) | [publisher_ergonomics.md](publisher_ergonomics.md) |
| Can a static-path receiver stop the rest of a publication? (#9) | **`bool receive()`**, where `false` stops. Detected at compile time and short-circuited by a fold | `=` on every build and both forms; no shared or thread-local state | `receive(const T&, Delivery&)` token (`=`). Rejected: a `cancel()` flag (thread-local: gcc +3.0, Cortex-M33 +14 path instr, +257 B RAM and TLS; static, single-threaded only: gcc +2.0, Cortex-M33 +5) and filter-based control (gcc +6.3, Cortex-M33 +11) | [static_cancellation.md](static_cancellation.md) |
| How do static wiring and runtime subscribers meet? (#9) | **`DynamicPort<T, N>`**: an intrusive slot array bound as one more receiver of the static wiring | `=` against a hand-written registry with the same features, populated and empty, on every build | `BrokerPort<T>` over the #8 registry (lean configuration), for when the dynamic side needs policy: publish `=` on gcc/clang; setup/teardown +20/+54, about 1.5 KB image. Rejected: routing static receivers through the registry (+37 gcc, +10 clang) | [static_dynamic_bridge.md](static_dynamic_bridge.md) |
| What makes `disconnect()` safe against concurrent delivery? (#5) | **Mechanism 1, the handshake the #8 prototype already uses** | Correct in every probe, with unbounded threads and nesting. With the empty-table fast path all three now share: 0/1/8 subscribers 25/99/260 instr/op, create+destroy 121 | As submitted, hazard pointer and epoch failed the lifetime probes. Fixed (round 2) and given the same fast path: hazard 36/115/192/204, epoch 30/88/134/166; both bound threads (8) and nesting (4), drop publications past those bounds in release builds, and use 4 to 7x the table RAM | [quiescence.md](quiescence.md) sections 9-11 |
...
- **Quiescence.** The recommended mechanism and the runner-up both failed three lifetime probes written
  ...
  reversed. A second round fixed both and re-measured them with the fixes' cost included. That confirmed
  the handshake, and it showed the fixed mechanisms drop publications past their bounds in release builds.
...
- **Fairness review (2026-09).** Every comparison was re-checked for an equal-work reference and for each
  alternative in its best form. Runtime-bound, type-erased and registry-backed patterns had been compared
  with static-address code; they now have their own hand-written references (`handwritten_runtime`,
  `handwritten_erased`, `handwritten_registry`). Alternatives were improved where a careful user would write
  them better: publishers hold the wiring by value, `Wiring` reads each binding at its own delivery, transport
  adapters are held by value with compile-time split horizon, the #8 registry and today's API are also
  measured in their leanest configurations, the cancel flag can be static, and every quiescence mechanism has
  the same empty-table fast path. Outcome: B1, B3 and the `DynamicPort` bridge cost nothing over hand-written
  code doing the same job; K11 and K12 are withdrawn; K14 (silent capability mismatch, which the review hit)
  and K15 (split-horizon precondition) are new. Details: COLLAPSE_EVIDENCE.md "Fairness review" and the
  review section at the end of each spike document.
