...
| What makes `disconnect()` safe against concurrent delivery? (#5) | **Mechanism 1, the handshake the #8 prototype already uses** | Correct in every probe, with unbounded threads and nesting. 0/1/8 subscribers: 67/95/256 instr/op; create+destroy 108 | As submitted, hazard pointer and epoch failed the lifetime probes (self-disconnect deadlock; nested publish and a 9th thread were use-after-free windows). Fixed in round 2: hazard 100/111/188/198 and epoch 57/84/130/163, but both bound threads (8) and nesting (4), drop publications past those bounds in release builds, and use 4 to 7x the table RAM | [quiescence.md](quiescence.md) sections 9-10 |
