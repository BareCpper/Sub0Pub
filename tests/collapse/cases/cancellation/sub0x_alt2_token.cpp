/** Cancellation Alt 2: a cancellation token, sub0x::Delivery&, optionally accepted as a second receive()
 *  parameter (tests/collapse/sandbox/sub0x_static.hpp). Controller and Logger keep the plain receive(const T&)
 *  and never see the token. Bound order: Gate, Controller, Logger (StaticWiring, pattern B2). */
...
struct Gate {
    void receive(const Sample& s, sub0x::Delivery& delivery) noexcept
    {
...
template<class Out>
struct Sensor {
    void send(uint32_t v) noexcept { Out::publishWithToken(Sample{v}); }
};
